...
按 `Enter` 进入选中仓库的详情面板，显示别名、名称、路径、描述、远程地址、标签和最后扫描时间。详情面板内裸键操作保持不变：`e` 编辑别名，`t` 管理标签，`v`/`u`/`a`/`o` 打开编辑器，`y` 复制路径。按 `Ctrl+C` 或 `Esc` 返回列表。

### 搜索

按 `/` 展开搜索输入框，输入内容即可实时过滤仓库。搜索范围涵盖名称、别名、描述、路径、远程 URL 和标签，大小写不敏感。

- 按 `Enter` 隐藏输入框，保留过滤结果（搜索前缀旁显示当前关键词）
