...
| `y` | 复制仓库路径到剪贴板 |
| `r` | 重新扫描文件系统 |
| `Esc` | 取消编辑 / 返回列表 / 清空搜索 / 退出 |
| `q` | 退出 |

### 详情面板

按 `Enter` 进入选中仓库的详情面板，显示别名、名称、路径、描述、远程地址、标签和最后扫描时间。详情面板内可执行所有操作：`e` 编辑别名，`t` 管理标签，`v`/`u`/`a`/`o` 打开编辑器，`y` 复制路径。按 `Esc` 返回列表。

### 别名与标签编辑

在列表或详情面板中按 `e` 进入别名编辑模式，直接输入新别名，`Enter` 保存，`Esc` 取消。留空则清除别名。

按 `t` 进入标签管理模式。输入标签名后按 `Enter` 添加；按数字键 `1`–`9` 移除对应序号的标签。按 `Esc` 退出管理模式。

### 搜索

按 `/` 聚焦搜索栏，输入内容即可实时过滤仓库。搜索范围涵盖名称、别名、描述、路径、远程 URL 和标签，大小写不敏感。
