"""Configuration constants and helpers for blink loop."""
...
def ensure_tloop_home():
    ...
    if not TASKS_FILE.exists():
        ...
        print(f"\nEdit ~/.blink/loop/tasks.yaml to define your tasks, then run blink run.")
        sys.exit(0)
...
def load_config():
    if not TASKS_FILE.exists():
        ...
        print(f"Run blink run to initialize, then edit tasks.yaml.")
        sys.exit(1)
    try:
