...
- `logger.py` — Daily-rotation application logger writing to `~/.blink/logs/blink-YYYY-MM-DD.log`. Format: `[HH:MM:SS]-[module] message`. Thread-safe via lock. `log(module, message)` for single lines, `log_lines(module, content)` for multi-line content (used for AI input/output). Integrated in: review (AI prompt + output), claude_runner (call tracking), TUI (commit, pull, scan, task operations).
...
- Code review uses three-layer context: diff + temporary merged branch + project rules (`docs/blink/review-rules.md`). Reports are generated in **中文** via `REVIEW_PROMPT` (VERDICT keywords remain English for `parse_verdict()` regex). `REVIEW_PROMPT` is designed for anti-hallucination: explicitly scopes review to diff content only, requires evidence citation (`**依据**` field quoting diff code), introduces `[疑似]` tag for uncertain findings (severity downgraded), and excludes style/naming/comments from review scope. Report header uses Chinese labels (分支/基准/日期/结论). Reports saved to `<project>/docs/blink/code-review/<branch-slug>-<date>.md`. `run_claude_text()` must NOT append EXECUTION_SUFFIX — review is analysis, not execution. `create_review_branch()` returns 4-tuple `(review_name, original_branch, stashed, error)`; `error` is `None` on success, `(error_type, message)` on failure where `error_type` is `"conflict"` (genuine merge conflict — generates DENY report immediately) or `"error"` (other merge failure — falls back to diff-only mode). Conflict detection checks for `"CONFLICT"` in merge output, not just returncode. TUI Shift+V fetches recent 5 branches via `git_ops.get_recent_branches()` (sorted by committer date, excludes main/master), user selects with ←→/Enter; verdict displayed in status bar with 5s auto-dismiss. Shift+L opens last report in IDE. `detect_main_branch()` checks `main` then `master` when `--against` is not specified. Branch slug replaces `/` with `-` and collapses consecutive `-`.
