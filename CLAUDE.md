...
- `detail.py` — `DetailPanel` class rendering full repo info (alias, name, path, description, remotes, status, tags, pinned, actions). 13 selectable lines (including Pull Latest at index 4 and Status row at index 8). Supports inline alias edit, tag management popovers, pin toggle, and status copy.
...
| `Shift+U` | 拉取最新代码（状态栏显示旋转动画"正在拉取..."，完成后刷新状态徽标） | ✓ |
...
- **详情面板**（detail panel）— 13 行可选中，每行按 Enter 执行对应操作
  ...
  - 行 4（Pull Latest）— Enter 执行 `git pull`，状态栏显示 braille 旋转动画"正在拉取..."，完成后刷新状态徽标并提示结果（"✓ Pull complete" / "✓ Already up to date" / "✗ Pull failed: ..." / "✗ Pull timed out"）；无 remote 时提示"No remote configured"；detached HEAD 时提示"Detached HEAD"
  - 行 5（Name）— Enter 复制项目名称，状态栏提示
  - 行 6（Path）— Enter 复制路径，状态栏提示
  - 行 7（Git）— Enter 将 SSH 地址转为 HTTPS 并在浏览器打开
  - 行 8（Status）— Enter 复制状态摘要到剪贴板
  - 行 9（Pinned）— Enter 切换置顶状态，状态栏提示"已置顶"/"已取消置顶"
  - 行 10（Alias）— Enter 进入别名编辑态
  - 行 11（Tags）— Enter 进入标签编辑态
  - 行 12（Description）— Enter 进入描述编辑态
...
- Pull action (`_run_pull`) runs `git pull` in a background thread (30s timeout) with the same braille spinner pattern. `check_pull_prereqs(repo)` checks for remotes and detached HEAD before execution. `parse_pull_output(stdout, returncode, stderr)` parses git output into (success, message). `_pulling` flag prevents concurrent pulls; independent from `_committing` flag. Pull spinner has priority over commit spinner in `_status_text()`. On completion, status refreshes and result message displays for 3 seconds. Detail panel delegates pull to app via `on_pull` callback.
